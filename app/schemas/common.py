...
class HealthResponse(BaseModel):
    ...
    status: str = Field(
        ..., description="Statut de l'API (healthy, degraded, unhealthy)"
    )
    timestamp: datetime = Field(
        default_factory=datetime.now, description="Horodatage de la vérification"
    )
...
class StatusResponse(BaseModel):
    ...
    uptime_seconds: Optional[float] = Field(
        None, description="Temps de fonctionnement en secondes"
    )
...
class ErrorResponse(BaseModel):
    ...
    detail: Optional[Dict[str, Any]] = Field(
        None, description="Informations supplémentaires sur l'erreur"
    )
    timestamp: datetime = Field(default_factory=datetime.now)
